...
// Função para liberar toda a memória alocada para as tarefas
void free_all_tasks() {
    Tarefa* current = head;
    Tarefa* next;
    while (current != NULL) {
        next = current->prox; // Salva o próximo nó antes de liberar o atual
        free(current);        // Libera a memória do nó atual
        current = next;       // Move para o próximo nó
    }
    head = NULL; // Garante que a lista esteja vazia após a liberação
    tail = NULL;
    tamanho = 0;
}
    ...
    while(1) {
        ...
        switch(entr) {
            ...
            case 4:
                ...
                break;
            case 5:
                // Sair do programa e liberar memória
